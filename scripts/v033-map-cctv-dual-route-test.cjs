...
ok(html.includes('0.34.0 SEARCH FUSION + MAP CCTV'),'v0.33 build label');
...
ok(app.includes('state.routeCandidates = chooseTwoNavigationRoutes(routePool)'),'route planner keeps two comparable routes');
ok(!app.includes('national-hotspot-tag ${h.status'),'large national numeric hotspot labels removed');
ok(sw.includes('eye-taiwan-shell-v340'),'service worker cache v330');
